...
class Session
{
	...
	public static function createUser($login, $password, $password_confirm, $User)
	{
		...
		{
			throw new ExceptionInvalidUser("Password muss mindesten f�nf Zeichen lang sein.");
		}
		...
		{
			throw new ExceptionInvalidUser("Login ung�ltig.");
		}
		
		// Check if both password strings are equal.
		if ($password != $password_confirm)
		{
			throw new ExceptionInvalidUser("Die Eingaben f�r das Passwort unterscheiden sich.");
		}

		// Check user name
		if ($db->QueryResult("SELECT * FROM user WHERE name = '$User->name';") != null)
		{
			throw new ExceptionInvalidUser("Name ung�ltig.");
		}
		...
		if (strlen($User->email) == 0 ||
			...
			substr_count($User->email, '.') < 1)
			{
				throw new ExceptionInvalidUser("E-Mail-Adresse ung�ltig.");
			}
	...
	public static function changeUserPassword($userid, $oldpassword, $newpassword, $newpassword2)
	{
		...
		{
			throw new ExceptionInvalidUser("Die Eingaben f�r das neue Passwort unterscheiden sich.");
		}
